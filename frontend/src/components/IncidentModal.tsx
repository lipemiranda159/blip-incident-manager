...
interface IncidentModalProps {
  ...
  onAssignedToUpdate: (incidentId: string, assignedToId: string | null) => void
}
...
export const IncidentModal = ({
  ...
  const [currentAssignedTo, setCurrentAssignedTo] = useState(incident.assignedTo?.id || null)
  const [hasChanges, setHasChanges] = useState(false)


  // Note: Removed useBodyScrollLock as BdsModal handles scroll management internally
  ...
  const handleStatusChange = (e: CustomEvent) => {
    const newStatus = e.detail.value as IncidentDto['status']
    setCurrentStatus(newStatus)
    setHasChanges(newStatus !== incident.status || currentAssignedTo !== (incident.assignedTo?.id || null))
  }

  const handleAssignToMe = () => {
    setCurrentAssignedTo(currentUser.id)
    setHasChanges(currentStatus !== incident.status || currentUser.id !== (incident.assignedTo?.id || null))
  }

  const handleUnassign = () => {
    setCurrentAssignedTo(null)
    setHasChanges(currentStatus !== incident.status || null !== (incident.assignedTo?.id || null))
  }
  ...
  const handleSave = async () => {
    ...
    setIsSubmitting(true)
    try {
      if (currentStatus !== incident.status) {
        await onStatusUpdate(incident.id, currentStatus)
      }
      if (currentAssignedTo !== (incident.assignedTo?.id || null)) {
        await onAssignedToUpdate(incident.id, currentAssignedTo)
      }
      setHasChanges(false)
      ...
      <div style={{ maxHeight: '75vh', overflowY: 'auto', padding: '16px' }}>
            ...
            <BdsGrid direction="row" gap="1" align-items="center">
              ...
              <BdsGrid direction="column" gap="1">
                ...
                {currentAssignedTo ? (
                  <BdsGrid direction="row" gap="1" align-items="center">
                    <BdsTypo variant="fs-14" bold="semi-bold">
                      {currentAssignedTo === currentUser.id ? currentUser.name :
                        (currentAssignedTo === incident.assignedTo?.id ? incident.assignedTo?.name : 'Usuário desconhecido')}
                    </BdsTypo>
                    {currentUser.type === 'atendente' && (
                      <>
                        {currentAssignedTo !== currentUser.id && (
                          <BdsButton
                            size="short"
                            variant="secondary"
                            onBdsClick={handleAssignToMe}
        ...
        <BdsGrid direction="column" gap="1">
          ...
          <BdsGrid direction="row" gap="1">
            <BdsTypo variant="fs-14" bold="extra-bold" color="primary">
              Descrição
            </BdsTypo>
            <BdsGrid padding="1">
          ...
          {/* Comments Section */}
          <BdsGrid direction="column" gap="2" md="8">
          ...
          <BdsGrid direction="row" gap="2" flex-wrap="wrap" align-items="flex-start">
            ...
            <BdsGrid direction="row" gap="2" xxs="12" sm="6">
              ...
              {(incident.status !== 'Resolvido' && incident.status !== 'Cancelado') ? (
                <BdsSelect value={currentStatus} onBdsChange={handleStatusChange}>
